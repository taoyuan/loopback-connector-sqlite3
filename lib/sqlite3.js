...
SQLite3.prototype._buildColumnIndexes = function(model) {
  ...
  function indexStatement(index) {
    var unique = index.unique === true ||
      (index.options && index.options.unique === true);
    return 'CREATE ' + (unique ? 'UNIQUE' : '') +
      ' INDEX IF NOT EXISTS %s ON %s (%s)';
  }
  ...
  for (var propertyName in properties) {
    ...
    if (property.index)
      indexes.push(fmt(indexStatement(property.index),
  ...
  if (settings && settings.indexes) {
    for (var index in settings.indexes) {
      ...
      indexes.push(fmt(indexStatement(settings.indexes[index]),
